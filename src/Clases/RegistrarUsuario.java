...
public class RegistrarUsuario {
    ...
    public boolean agregarUsuario(String nom, char genero,long fechaNac,int tel,long fechaEntrada,String email,String contra){
        try{
            ...
            System.out.println("Errores dd: "+e.getMessage());
        }       
        return false;
    }
    ...
    private void crearCarpetaUser(String correo) throws IOException{
        ...
        f.createNewFile();
        f=new File("Users\\"+correo+"\\postsInicio.fbn");
        f.createNewFile();
    }
    ...
    public boolean IniciarSesion(String correo , String contra){
        try{
            gerencia.seek(0);
            while(gerencia.getFilePointer()<gerencia.length()){
                String mail=gerencia.readUTF();
                String pass=gerencia.readUTF();
                boolean t=gerencia.readBoolean();
                if(mail.equals(correo) && pass.equals(contra) && t){
                   return true;     
                }
            }
        }catch(IOException e){
    ...
    public void desactivarCuenta(String correo) throws IOException{
        gerencia.seek(0);
        while(gerencia.getFilePointer()<gerencia.length()){
            if(correo.equals(gerencia.readUTF())){
                gerencia.readUTF();
                gerencia.writeBoolean(false);
                break;
            }else{
                gerencia.readUTF();
                gerencia.readBoolean();
    ...
    public boolean verificarActivo(String correo) throws IOException{
        gerencia.seek(0);
        while(gerencia.getFilePointer()<gerencia.length()){
            String g=gerencia.readUTF();
            gerencia.readUTF();
            boolean t=gerencia.readBoolean();
            if(g.equals(correo) && t){
                return true;
            }
        }
        return false;
